...
def get_transaction_data(iban, access_token, x_request):
    ...
    """
    url = f"https://sandbox.api.bnpparibasfortis.com/psd2/v3/accounts/{iban}EUR/transactions?brand=fintro"
    headers = {
...
def get_balance_data(iban, access_token, x_request):
    ...
    """
    url = f"https://sandbox.api.bnpparibasfortis.com/psd2/v3/accounts/{iban}EUR/balances?brand=fintro"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Signature": "Only for PROD",
        "X-Request-ID": x_request,
