...
from HB_requests import get_access_token, get_authorization_code, get_balance_data, get_ibans, get_new_access_token, get_transaction_data, get_account_data
...
url_auth = f"https://sandbox.auth.bnpparibasfortis.com/authenticate?client_id={app_id}&brand=hb"
payload_auth = {
    "username": user,
    "password": password,
    "next": "",
...
url = "https://sandbox.api.bnpparibasfortis.com/psd2/v3/accounts?brand=hb"
headers = {
...
url_authorize = f"https://sandbox.auth.bnpparibasfortis.com/authorize?client_id={app_id}&state=api&brand=hb"
headers_authorize = {
    "Authorization": f"Bearer {access_token}",
...
if new_access_token:
    ...
    for iban in iban_list:
        ...
        url_transactions = f"https://sandbox.api.bnpparibasfortis.com/psd2/v3/accounts/{iban}EUR/transactions?brand=hb"

        transaction = get_transaction_data(iban, new_access_token, x_request)
        transaction_data.append(transaction)

    for iban in iban_list:
        url_balances = f"https://sandbox.api.bnpparibasfortis.com/psd2/v3/accounts/{iban}EUR/balances?brand=hb"
