import subprocess
import datetime

# The script uses the downloaded example eIDAS certificates
keyId = "SN=546212fb"  # Serial number of the downloaded certificate in hexadecimal code
certPath = "./certs/"  # Path of the downloaded certificates and keys
httpHost = "https://api.sandbox.ing.com"

# httpMethod value must be in lower case
httpMethod = "post"
reqPath = "/oauth2/token"

# You can also provide scope parameter in the body E.g. "grant_type=client_credentials&scope=greetings%3Aview"
# scope is an optional parameter. If you don't provide a scope, the accessToken is returned for all available scopes
payload = "grant_type=client_credentials"
payloadDigest = subprocess.check_output(f'echo -n "{payload}" | openssl dgst -binary -sha256 | openssl base64', shell=True).decode().strip()
digest = f"SHA-256={payloadDigest}"

reqDate = datetime.datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S GMT")

# signingString must be declared exactly as shown below in separate lines
signingString = f"(request-target): {httpMethod} {reqPath}\n" \
                f"date: {reqDate}\n" \
                f"digest: {digest}"

signature = subprocess.check_output(f'printf %s "{signingString}" | openssl dgst -sha256 -sign "{certPath}example_client_signing.key" -passin "pass:changeit" | openssl base64 -A', shell=True).decode().strip()

# Curl request method must be in uppercase e.g "POST", "GET"
curl_command = f'curl -i -X POST "{httpHost}{reqPath}" ' \
               f'-H "Accept: application/json" ' \
               f'-H "Content-Type: application/x-www-form-urlencoded" ' \
               f'-H "Digest: {digest}" ' \
               f'-H "Date: {reqDate}" ' \
               f'--user-agent "openbanking-get-started/1.0.0 bash" ' \
               f'-H "TPP-Signature-Certificate: -----BEGIN CERTIFICATE----------END CERTIFICATE-----" ' \
               f'-H "authorization: Signature keyId=\\"{keyId}\\",algorithm=\\"rsa-sha256\\",headers=\\"(request-target) date digest\\",signature=\\"{signature}\\"" ' \
               f'-d "{payload}" ' \
               f'--cert "{certPath}example_client_tls.cer" ' \
               f'--key "{certPath}example_client_tls.key"'

subprocess.run(curl_command, shell=True)